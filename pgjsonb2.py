...
def returner(ret):
    ...
    '''
    if not ret['fun'] in ('test.ping', 'saltutil.find_job', 'runner.manage.up', 'runner.manage.down'):
        try:
            with _get_serv(ret, commit=True) as cur:
                changes = False
                for fun, data in ret['return'].items():
                    if len(data['changes']) > 0 and data['result']:
                        changes = True
                sql = '''INSERT INTO salt_returns
                        (fun, jid, return, id, success, full_ret, alter_time, changes)
            ...
            log.critical('Could not store return with pgjsonb2 returner. PostgreSQL server unavailable.')
